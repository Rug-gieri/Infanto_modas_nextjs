...
import { deleteCliente, listClientes, listPedidosByCliente, updateCliente } from '@/lib/services/clientes'
import type { Cliente, ClientePedido } from '@/lib/types/admin'
...
export default function AdminClientes() {
  ...
  async function fetchClientes() {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Erro ao buscar clientes.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Erro ao excluir cliente.'))
    }
  }
  ...
  async function openPedidos(cliente: Cliente) {
    ...
    try {
      setPedidos(await listPedidosByCliente(cliente.id))
    } catch (err) {
  ...
  function handleExportCSV() {
    const rows = filtered.map((c) => [c.nome, c.email, formatPhone(c.telefone), c.aceita_newsletter ? 'Sim' : 'Não', formatDate(c.criado_em)].join(','))
  ...
  const filtered = clientes.filter((c) => c.nome.toLowerCase().includes(searchTerm.toLowerCase()) || c.email.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    <div className="max-w-[1200px] mx-auto py-8 px-4">
      ...
      <div className="relative mb-6">
        ...
        <Input type="text" placeholder="Buscar por nome ou e-mail..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
        {searchTerm && (
      ...
      <Card className="rounded-2xl border-border overflow-hidden p-0">
        {loading ? (
          ...
          <Table>
            <TableCaption className="pb-4">Mostrando {filtered.length} de {clientes.length} clientes</TableCaption>
            <TableHeader>
              <TableRow>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Checkbox id="edit-newsletter" checked={editNewsletter} onCheckedChange={(checked) => setEditNewsletter(checked === true)} />
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Tem certeza que deseja excluir <strong>{deletingCliente?.nome}</strong>? Pedidos associados serão preservados.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
